...
class RectBox(ShapeBuilder):
    ...
    def default_sizes(cls, scale):
        ...
        # ratio has to go through this function
        return dict(width=size, height=size)

    @classmethod
    # use ratio instead of height
    def _build(cls, width, height):
        vertices = []
        for i in range(4):
            vx = (1 - 2 * (i in (1, 2))) / 2. * width 
            vy = (1 - 2 * (i in (2, 3))) / 2. * height * 1.5
            vertices.append((vx, vy))
        return vertices_to_polygon(vertices)  
...
class Triangle(ShapeBuilder):
    ...
    def default_sizes(cls, scale):
        ...
        scale_range = [0., SCENE_WIDTH]
        size = _interpolate(scale_range, scale)
        return dict(side=size)

    @classmethod
    def _build(cls, side):
        vertices = [(0.,side*math.sqrt(3)/4.),(side*(-0.5),side*(-math.sqrt(3)/4.)),((side*0.5),side*(-math.sqrt(3)/4.))]
        return vertices_to_polygon(vertices)

class IsoTriangle(ShapeBuilder):
    ...
    @classmethod
    def _build(cls, side):
        vertices = [(-side/3.,-side/3.),(2*side/3., -side/3.),(-side/3., 2*side/3.)]
        return vertices_to_polygon(vertices)
...
class LyingSticks(ShapeBuilder):
...
#
class Catapult(ShapeBuilder):

    @classmethod
    def default_sizes(cls, scale):

        # set scale range and interpolate according to the object
        scale_range = [0., SCENE_WIDTH]
        width = _interpolate(scale_range, scale)
        ...
        return dict(
            ...
            width=width,
            thickness=thickness,
            base_width=width * 1.00,
        )

    @classmethod
    def _build(cls, height, width, thickness, base_width):

        # vertices of the base of the catapult
        vertices = []
        for i in range(4):
