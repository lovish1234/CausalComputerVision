...
def build_task(C, bar_y, vbar_x_scale, distance_to_wall, angle, over):
    ...
    for i in range(10):
        bar = C.add(
            ...
            right=shape.left - 2 - distance_to_wall * C.scene.height)
        if bar.bottom > 0:
            bars.append(bar)

    if over:
        shape.set_left(bars[0].right + 1)
    else:
        shape.set_right(C.scene.width - 10)
        shape.set_bottom(random.uniform(0.1,1)*C.scene.height)
    ...
    if not 0.2 < hole_top < 0.9:
        raise creator_lib.SkipTemplateParams
    hole_size = shape.width / C.scene.width * 2
    C.add(
        'static bar',
        scale=1.0 - hole_top,
        angle=90,
