...
@creator_lib.define_task_template(
    ...
    shape_size=np.linspace(0.05, 0.15, 5),
    left=[True, False],
    search_params=dict(
    ...
    over=[True,False],
    version='4')
def build_task(C, hole_left, height, left, shape_size, over):
    # Add ball.
    if left:
        shape_center = hole_left
    else:
        shape_center = 1 - hole_left
    ...
    if over:
        ...
        shape = C.add(
            'dynamic ball',
            scale=shape_size,
            center_x=shape_center * C.scene.width,
    ...
    if over:
        ...
        target = left_floor if left else right_floor
