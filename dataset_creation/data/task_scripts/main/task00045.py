...
def build_task(C, hole_left, bar_height, ball_distance, confounder, over):
    ...
    if over:
        # Add balls.
        # randomize the height of ball
        height = random.uniform(bar_height+__BALL_SIZE, 1.0-__BALL_SIZE)
        #print (over,height)
        shape = C.add('dynamic box', scale=__BALL_SIZE) \
                .set_center_x(0.5 * C.scene.width) \
                .set_bottom(height * C.scene.height)
    else:
        # randomize the height of ball
        height = random.uniform(0.0+__BALL_SIZE, bar_height-__BALL_SIZE)
        #print (over,height)
        shape = C.add('dynamic ball', scale=__BALL_SIZE) \
                .set_center_x(0.5 * C.scene.width) \
                .set_bottom(height * C.scene.height)
    ...
    if shape.left >= left_bar.right and shape.right <= right_bar.left:
        raise creator_lib.SkipTemplateParams
